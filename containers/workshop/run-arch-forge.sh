...
redraw() {
    ...
    printf "  ${BOLD}${CYAN}arch-forge-runner${RESET}  ${DIM}iter %d/%d${RESET}\n" \
        "$iter" "$max"
...
run_iteration() {
    ...
    LAST_OUTPUT="/tmp/arch-forge-runner-output.$$"
    > "$LAST_OUTPUT"
    TAIL_BUFFER=()
...
probe_limit() {
    ...
    local probe_output="/tmp/arch-forge-probe-output.$$"
    docker exec --user node "$CONTAINER_NAME" \
...
cleanup() {
    rm -f "/tmp/arch-forge-runner-output.$$" "/tmp/arch-forge-probe-output.$$"
}
trap cleanup EXIT
...
main() {
    ...
    printf "\n${BOLD}${CYAN}  arch-forge-runner${RESET}\n"
