...
CONTAINER="arch-tool-mount-test-$$"
TOPIC_A="/tmp/arch-mount-test-topic-a-$$"
TOPIC_B="/tmp/arch-mount-test-topic-b-$$"
...
docker exec --user node "$CONTAINER" cat /workspace/prompt.md | grep -q "topic-a prompt" \
    || { echo "FAIL: should see topic A prompt.md"; exit 1; }

docker exec --user node "$CONTAINER" cat /workspace/progress.md | grep -q "topic-a progress" \
    || { echo "FAIL: should see topic A progress.md"; exit 1; }

# Container does NOT see topic B files
ls_output=$(docker exec --user node "$CONTAINER" ls /workspace/ 2>&1)
...
docker exec --user node "$CONTAINER" test -d "$TOPIC_B" 2>/dev/null \
...
docker exec --user node "$CONTAINER" bash -c "echo 'new-file' > /workspace/output.md"
...
# poc dir is created inside topic workspace by entrypoint
docker exec --user node "$CONTAINER" test -d /workspace/poc \
    || { echo "FAIL: entrypoint should create /workspace/poc"; exit 1; }

# poc dir appears in host topic A dir
test -d "$TOPIC_A/poc" \
    || { echo "FAIL: poc/ should appear in host topic A dir"; exit 1; }
